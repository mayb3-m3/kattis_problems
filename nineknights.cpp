...
void solve(){
    ...
    for(int i = 0; i < 5; i++){
        for(int j = 0; j < 5; j++){
            if(vtr[i][j] == 'k'){
                count++;
                if(i-2 >= 0 && j+1 <= 4 && vtr[i-2][j+1] == 'k')
                    ans = 0;
                if(i-2 >= 0 && j-1 >= 0 && vtr[i-2][j-1] == 'k')
                    ans = 0;
                if(i+2 <= 4 && j+1 <= 4 && vtr[i+2][j+1] == 'k')
                    ans = 0;
                if(i+2 <= 4 && j-1 >= 0 && vtr[i+2][j-1] == 'k')
                    ans = 0;
                if(j+2 <= 4 && i+1 <= 4 && vtr[i+1][j+2] == 'k')
                    ans = 0;
                if(j+2 <= 4 && i-1 >= 0 && vtr[i-1][j+2] == 'k')
                    ans = 0;
                if(j-2 >= 0 && i+1 <= 4 && vtr[i+1][j-2] == 'k')
                    ans = 0;
                if(j-2 >= 0 && i-1 >= 0 && vtr[i-1][j-2] == 'k')
                    ans = 0;
            }
        }
    }
